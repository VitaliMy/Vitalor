...
if args.type == "annuity": 
    if (args.payment is not None) and (args.periods is not None) and (args.interest is not None): 
        ...
        credit_principal = int(payment / ((nominal_interest * ((1 + nominal_interest) ** periods)) / (((1 + nominal_interest) ** periods) - 1))) 
        print("Your credit principal =", str(credit_principal) + "!") 
        print("Overpayment =", round((payment * periods) - credit_principal)) 
        ...
        interest = args.interest 
        nominal_interest = (interest / 100) / 12
        annuity = int(principal * ((nominal_interest * ((1 + nominal_interest) ** periods)) / (((1 + nominal_interest) ** periods) - 1))+1) 
        print("Your annuity payment =", str(annuity) + "!")
        print("Overpayment =", int((annuity * periods) - principal)) 
    else: 
    ...
    if (args.principal == None) or (args.periods == None) or (args.interest == None): 
        ...
        for n_periods in range(1, args.periods + 1): 
            d = math.ceil( (args.principal / args.periods) + nominal_interest * (args.principal - args.principal * (n_periods - 1) / args.periods))
